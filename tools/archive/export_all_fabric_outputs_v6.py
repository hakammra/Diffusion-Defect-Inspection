"""Export all 110 fabric test images for V6 evaluation gallery with Color Composite Overlays."""
...
V6_DIR = ROOT / "artifacts" / "fabric_stain_evaluation_v6"
NPZ_PATH = V6_DIR / "test_maps_v6_float16.npz"
CSV_PATH = V6_DIR / "per_image_v6.csv"
DATA_ZIP_PATH = ROOT / "output" / "datasets" / "fabric_stain_pilot.zip"
GALLERY_DIR = V6_DIR / "gallery"
...
def analyze_defect_clusters_v6(v6_map: np.ndarray, tau_high: float = 15.0, tau_low: float = 3.3,
                               min_area: int = 30, linear_thresh: float = 0.80):
    ...
    # Consolidate stain mask with 2-step morphological close
    if np.any(stain_mask):
        padded = np.pad(stain_mask, 2, mode="constant")
        # simple binary dilation then erosion
        dilated = padded.copy()
        for _ in range(2):
            dilated = (
...
def run():
    if not NPZ_PATH.is_file():
        print(f"NPZ not found at {NPZ_PATH}; run after V6 Kaggle results are extracted.")
        return
    ...
    v6_maps = data["v6_maps"].astype(np.float32)
    ...
    print(f"Generating V6 gallery for {len(rows)} test images...")

    for idx, r in enumerate(rows):
        img_rel = r["image"]
        kind = r["kind"]
        score = float(r["v6_score"])
        ...
        v6_map = v6_maps[idx]
        heatmap_im = colorize_heatmap(v6_map, vmin=0.0, vmax=max(30.0, score * 0.8))

        has_s, has_c, m_s, m_c, m_tot, clusters = analyze_defect_clusters_v6(
            v6_map, tau_high=tau_high, tau_low=tau_low, min_area=min_area, linear_thresh=0.80
        )
        ...
        # Categorization
        if not is_stain:
            category_tag = "crease" if has_c else "normal"
        else:
            category_tag = "strict" if strict_detected else ("recovered" if has_s else ("crease" if has_c else "missed"))
        ...
        if not is_stain and not has_s and not has_c:
            ...
            status_badge = "<span class='badge badge-crease'>Physical Fold Crease</span>"
            ...
            status_badge = "<span class='badge badge-missed'>Subtle / Missed</span>"
        ...
        <div class="card" data-category="{category_tag}">
            ...
            <div class="card-body">
                ...
                <div class="panel">
                    <img src="images/{heat_file}" alt="V6 Multi-Scale Heatmap">
                    <div class="label">V6 Heatmap</div>
                </div>
                <div class="panel">
...
<head>
    ...
    <title>Fabric Stain V6 Complete Outputs Gallery</title>
    <style>
...
<body>
    <h1>Fabric Stain Inspection &mdash; Complete V6 Output Gallery (110 Test Images)</h1>
    <div class="stats-bar">
        ...
        <div class="stat-item"><span class="stat-value">100</span><span class="stat-label">Stained Samples</span></div>
        <div class="stat-item"><span class="stat-value" style="color: #60A5FA;">54/100</span><span class="stat-label">Strict Detections (&tau;=18.8)</span></div>
        <div class="stat-item"><span class="stat-value" style="color: #34D399;">71/100</span><span class="stat-label">Total Defects Captured</span></div>
    ...
    <div class="filter-bar">
        ...
        <button onclick="filterCards('normal')">Clean Normals (9)</button>
        <button onclick="filterCards('strict')">Strict Stains (54)</button>
...
"""
    ...
    print(f"\nV6 Gallery successfully created at:\n{index_html_path}")
