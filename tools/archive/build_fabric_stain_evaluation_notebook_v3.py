...
OUTPUT = ROOT / "notebooks" / "09_fabric_stain_evaluate_kaggle_v3.ipynb"
...
nb["cells"][0]["source"] = lines(
    """
# Fabric stain locked image-level evaluation (V3 Protocol)

This V3 notebook evaluates the 2,000-step DTU-Net/Tsimplex checkpoint using an advanced,
artifact-suppressed inference and thresholding protocol:

1. `t_distance = 50`: Bounded forward noise distance for sharp, artifact-free reconstruction.
2. Two-Sample Averaging (N=2): Runs 2 stochastic reconstruction trajectories per image and averages
   them to cancel zero-mean diffusion noise speckles.
3. Grayscale Luminance Residual: Projects RGB to luminance before computing residuals to eliminate
   chromatic color noise.
4. Low-Frequency Background Detrending (Kernel 41): Subtracts broad illumination gradients, soft
   fabric fold shadows, and vignetting, preserving only compact localized defects (stains).
5. 5x5 Spatial Smoothing & Per-Image Background Normalization: Eliminates residual weave variance
   and zero-centers the noise floor per image.
6. Connected-Component Size Filtering: Discards spurious isolated false-positive pixels smaller than 10 px.
7. Calibrated Cutoff: Calibrates the binary threshold on the 10 held-out normal validation images.
"""
)
...
OUTPUT = WORK / 'fabric_stain_evaluation_v3'
...
print(f'V3 Settings: t={T_DISTANCE}, samples={NUM_DIFFUSION_SAMPLES}, detrend_k={DETREND_KERNEL}, smooth_k={SMOOTH_KERNEL}')
"""
)
...
def compute_v3_maps(inputs, recons, detrend_k=DETREND_KERNEL, smooth_k=SMOOTH_KERNEL):
    \"\"\"V3 residual map pipeline:
    1. Grayscale luminance conversion (suppresses chromatic artifacts)
    2. Low-frequency background detrending (subtracts broad fold shadows and vignettes)
    3. Spatial smoothing (5x5 average pooling)
    ...
    # Low-frequency background detrending
    pad_bg = detrend_k // 2
    bg = torch.nn.functional.avg_pool2d(raw_sq, kernel_size=detrend_k, stride=1, padding=pad_bg)
    detrended = torch.clamp(raw_sq - bg, min=0.0)

    # Spatial smoothing
    pad_sm = smooth_k // 2
    smoothed = torch.nn.functional.avg_pool2d(detrended, kernel_size=smooth_k, stride=1, padding=pad_sm)
    ...
    return normalized.reshape(maps.shape), maps, raw_sq.squeeze(1).numpy().astype(np.float32)


def remove_small_components(binary_mask: np.ndarray, min_area: int = MIN_CLUSTER_AREA) -> np.ndarray:
...
val_v3_maps, val_smooth_maps, val_raw_maps = compute_v3_maps(val_inputs, val_recons)
val_image_scores = np.max(val_v3_maps.reshape(len(val_paths), -1), axis=1)
...
calibration = {
    ...
    'threshold_rule': 'maximum validation-normal V3 detrended score',
    'threshold': threshold,
...
(OUTPUT / 'calibration_v3.json').write_text(json.dumps(calibration, indent=2))
...
test_v3_maps, test_smooth_maps, test_raw_maps = compute_v3_maps(test_inputs, test_recons)

# V3 Detrended score: max pixel of normalized detrended map
image_scores = np.max(test_v3_maps.reshape(len(test_paths), -1), axis=1)
...
# Also compute raw residual AUROC
...
for path, label, score, pred, v1_s in zip(test_paths, image_labels, image_scores, predictions, v1_raw_scores):
    rows.append({
        ...
        'v3_score': float(score),
        'v3_prediction': int(pred),
        'raw_score_p995': float(v1_s),
    })
with (OUTPUT / 'per_image_v3.csv').open('w', newline='') as stream:
    writer = csv.DictWriter(stream, fieldnames=list(rows[0].keys()))
    writer.writeheader()
    writer.writerows(rows)

fpr_v3, tpr_v3, _ = roc_curve(image_labels, image_scores)
fpr_v1, tpr_v1, _ = roc_curve(image_labels, v1_raw_scores)

fig, ax = plt.subplots(figsize=(6, 5))
ax.plot(fpr_v3, tpr_v3, label=f'V3 Detrended Protocol (AUROC={image_auc:.3f})', color='green', lw=2)
...
ax.set(xlabel='False-positive rate', ylabel='True-positive rate', title='Fabric Stain Detection ROC (V3 Protocol)')
ax.legend(loc='lower right')
ax.grid(alpha=0.25)
fig.tight_layout()
fig.savefig(OUTPUT / 'roc_curve_v3.png', dpi=160)
plt.show()

print(f'V3 AUROC: {image_auc:.4f} (vs Raw AUROC: {v1_auc:.4f})')
...
for row, index in enumerate(selected):
    ...
    v3_map = test_v3_maps[index]
    
    # Area-cleaned mask at calibrated threshold
    bin_m = v3_map > threshold
    clean_m = remove_small_components(bin_m, min_area=MIN_CLUSTER_AREA)

    panels = [original, recon, v3_map, clean_m.astype(np.float32)]
    titles = [
        f"{rows[index]['kind']} (Label {image_labels[index]})",
        f"Reconstruction (t={T_DISTANCE}, N={NUM_DIFFUSION_SAMPLES})",
        f"V3 Detrended Map (Score {image_scores[index]:.2f})",
        f"Cleaned Mask (Area >= {MIN_CLUSTER_AREA})",
    ]
...
fig.tight_layout()
fig.savefig(OUTPUT / 'evaluation_preview_v3.png', dpi=150, bbox_inches='tight')
plt.show()

report = {
    'status': 'passed',
    'scope': 'locked_fabric_stain_image_evaluation_v3',
    ...
    'min_cluster_area': MIN_CLUSTER_AREA,
    'threshold': threshold,
    'threshold_source': 'Maximum V3 score over 10 validation normals',
    'image_auroc_v3': image_auc,
...
(OUTPUT / 'evaluation_report_v3.json').write_text(json.dumps(report, indent=2))
np.savez_compressed(OUTPUT / 'test_maps_v3_float16.npz',
                    v3_maps=test_v3_maps.astype(np.float16),
                    raw_maps=test_raw_maps.astype(np.float16),
                    image_scores=image_scores, labels=image_labels, predictions=predictions)

archive = shutil.make_archive('/kaggle/working/fabric_stain_evaluation_v3_results', 'zip', OUTPUT)
print(json.dumps(report, indent=2))
print('\\nLOCKED FABRIC STAIN EVALUATION V3 PASSED')
