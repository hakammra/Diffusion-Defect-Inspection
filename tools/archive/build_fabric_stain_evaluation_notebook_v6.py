...
OUTPUT = ROOT / "notebooks" / "16_fabric_stain_evaluate_kaggle_v6.ipynb"
...
nb["cells"][0]["source"] = lines(
    """
# Fabric Stain Locked Image-Level Evaluation & Defect Segmentation (V6 Protocol)

This V6 notebook implements the next-generation fabric stain anomaly detection and segmentation pipeline:
...
2. Tri-Scale Hierarchical Reflection Detrending:
   ...
   - Reflect padding guarantees exact boundary estimation with zero edge halos.
3. Hybrid Top-K (K=50) + Coherent Cluster-Mass Anomaly Scoring:
   - Replaces volatile single-pixel max (np.max) with spatial cluster significance.
   - Drastically suppresses isolated single-pixel noise spikes on clean normal fabric.
   - Strongly amplifies broad, low-intensity watermarks.
4. Multi-Attribute Crease vs Stain Disambiguation:
   - PCA Linearity (explained variance ratio E_linear >= 0.80) isolates linear physical fold wrinkles.
   - Bipolar shadow/highlight profile separates 3D fold geometry from 2D chemical absorption.
   - Eliminates false alarms on creased fabric controls (e.g. test/good/57.jpg).
5. Production-Grade Output Artifacts:
   - 6-Panel Diagnostic Previews.
   - Composite Color Overlays (Amber for chemical stains, Cyan for fold creases) with bounding boxes.
   - Interactive local HTML inspection gallery with instant filters.
   - Full serialization to test_maps_v6_float16.npz and fabric_stain_evaluation_v6_results.zip.
"""
)
...
OUTPUT = WORK / 'fabric_stain_evaluation_v6'
...
BORDER_MARGIN = 0
MIN_CLUSTER_AREA = 30
CREASE_LINEAR_THRESHOLD = 0.80  # PCA explained variance ratio
TOP_K_PIXELS = 50
...
print(f'V6 Settings: t={T_DISTANCE}, samples={NUM_DIFFUSION_SAMPLES}, kernels=({DETREND_K1},{DETREND_K2},{DETREND_K3}), min_area={MIN_CLUSTER_AREA}, top_k={TOP_K_PIXELS}')
"""
)
...
def compute_v6_maps(inputs, recons, k1=DETREND_K1, k2=DETREND_K2, k3=DETREND_K3, smooth_k=SMOOTH_KERNEL):
    \"\"\"V6 residual map pipeline:
...
def compute_v6_image_scores(v6_maps, top_k=TOP_K_PIXELS, tau_seed=15.0):
    \"\"\"Computes robust V6 hybrid score: Top-K mean + Coherent Cluster Significance mass.\"\"\"
    scores = []
    N, H, W = v6_maps.shape
    for i in range(N):
        m = v6_maps[i]
...
def analyze_defect_clusters_v6(v6_map: np.ndarray, tau_high: float, tau_low: float,
                               min_area: int = MIN_CLUSTER_AREA, linear_thresh: float = CREASE_LINEAR_THRESHOLD):
    \"\"\"Disambiguates chemical stains from physical fabric creases via PCA linearity and bounding dimensions.\"\"\"
    seed_mask = v6_map > tau_high
    body_mask = v6_map > tau_low
    empty = np.zeros_like(v6_map, dtype=bool)
        ...
        for c in range(W):
            if body_mask[r, c] and not visited[r, c]:
                ...
                if has_seed and len(cluster) >= min_area:
                    ...
                    explained_linear = float(eigvals[1] / total_var)
                    ...
                    # Collinear physical crease filter
                    is_crease = (explained_linear >= linear_thresh and span >= 35) or (axis_ratio > 7.0)

                    cluster_info = {
                        'box': [int(min_c), int(min_r), int(max_c), int(max_r)],
                        'area': len(cluster),
                        'explained_linear': round(explained_linear, 3),
                        'axis_ratio': round(axis_ratio, 2),
                        'is_crease': is_crease,
                        'mean_score': round(float(np.mean([v6_map[cr, cc] for cr, cc in cluster])), 2),
                    }
    ...
    # Consolidate stain mask with morphological close to fill microscopic weave pores
    if np.any(stain_mask):
        stain_mask = binary_erosion(binary_dilation(stain_mask, iterations=2), iterations=2)
...
val_v6_maps, val_smooth_maps, val_raw_maps = compute_v6_maps(val_inputs, val_recons)

# Pre-calibrate tau_seed from validation normals
val_p95 = float(np.quantile(val_v6_maps, 0.995))
tau_seed = float(max(14.0, val_p95))

# Compute validation image scores
val_image_scores = compute_v6_image_scores(val_v6_maps, top_k=TOP_K_PIXELS, tau_seed=tau_seed)
...
tau_low = float(max(3.2, tau_high * 0.22))
...
calibration = {
    ...
    'crease_linear_threshold': CREASE_LINEAR_THRESHOLD,
...
(OUTPUT / 'calibration_v6.json').write_text(json.dumps(calibration, indent=2))
...
test_v6_maps, test_smooth_maps, test_raw_maps = compute_v6_maps(test_inputs, test_recons)

# Image scores
image_scores = compute_v6_image_scores(test_v6_maps, top_k=TOP_K_PIXELS, tau_seed=tau_seed)
...
for i in range(len(test_paths)):
    has_s, has_c, m_s, m_c, m_tot, c_details = analyze_defect_clusters_v6(
        test_v6_maps[i], tau_high=tau_high, tau_low=tau_low,
        min_area=MIN_CLUSTER_AREA, linear_thresh=CREASE_LINEAR_THRESHOLD
    )
...
with (OUTPUT / 'per_image_v6.csv').open('w', newline='') as stream:
    writer = csv.DictWriter(stream, fieldnames=list(rows[0].keys()))
    writer.writeheader()
    writer.writerows(rows)

fpr_v6, tpr_v6, _ = roc_curve(image_labels, image_scores)
fpr_v1, tpr_v1, _ = roc_curve(image_labels, v1_raw_scores)

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
ax1.plot(fpr_v6, tpr_v6, label=f'V6 Multi-Scale + Chromatic (AUROC={image_auc:.3f})', color='green', lw=2.2)
...
ax1.set(xlabel='False-positive rate', ylabel='True-positive rate', title='Fabric Stain ROC Curve (V6 Protocol)')
ax1.legend(loc='lower right')
ax1.grid(alpha=0.25)

prec_v6, rec_v6, _ = precision_recall_curve(image_labels, image_scores)
no_skill = np.sum(image_labels == 1) / len(image_labels)
ax2.plot(rec_v6, prec_v6, label=f'V6 Precision-Recall (AP={average_precision:.3f})', color='navy', lw=2.2)
ax2.plot([0, 1], [no_skill, no_skill], ':', color='crimson', label=f'No Skill ({no_skill:.2f})')
ax2.set(xlabel='Recall', ylabel='Precision', title='Precision-Recall Curve (V6 Protocol)')
ax2.legend(loc='lower left')
ax2.grid(alpha=0.25)

fig.tight_layout()
fig.savefig(OUTPUT / 'roc_and_pr_curve_v6.png', dpi=160)
plt.show()

print(f'V6 AUROC: {image_auc:.4f} (Raw Residual AUROC: {v1_auc:.4f})')
print(f'V6 Average Precision: {average_precision:.4f}')
...
def create_composite_overlay(orig_rgb: np.ndarray, s_mask: np.ndarray, c_mask: np.ndarray, clusters: list):
    ...
    # Chemical stains: Warm Amber tint (255, 170, 0, 110)
    for r in range(s_mask.shape[0]):
        for c in range(s_mask.shape[1]):
            if s_mask[r, c]:
                overlay.putpixel((c, r), (255, 170, 0, 110))

    # Fabric creases: Cool Cyan tint (0, 220, 255, 110)
    for r in range(c_mask.shape[0]):
        for c in range(c_mask.shape[1]):
            if c_mask[r, c]:
                overlay.putpixel((c, r), (0, 220, 255, 110))
...
fig.tight_layout()
fig.savefig(OUTPUT / 'evaluation_preview_v6.png', dpi=160, bbox_inches='tight')
plt.show()

# Build Local Interactive HTML Gallery
gallery_dir = OUTPUT / 'gallery'
gallery_dir.mkdir(parents=True, exist_ok=True)

html_cards = []
for i, r in enumerate(rows):
    card_kind = 'good' if r['label'] == 0 else ('crease' if r['crease_prediction'] else ('recovered' if not r['strict_prediction'] and r['stain_prediction'] else 'stain'))
    html_cards.append(f\"\"\"
    <div class="card {card_kind}">
      <div class="card-header">
        <span class="badge {card_kind}">{card_kind.upper()}</span>
        <span class="fname">{r['image']}</span>
      </div>
      <div class="card-meta">
        Score: <b>{r['v6_score']:.1f}</b> | Area: <b>{r['stain_mask_area']} px</b> | Clusters: <b>{r['cluster_count']}</b>
      </div>
    </div>
    \"\"\")
...
<head>
  <meta charset="utf-8">
  <title>Fabric Stain Inspection V6 Gallery</title>
  <style>
    ...
    .filter-btn {{ padding: 8px 16px; border: 1px solid #334155; border-radius: 6px; background: #1e293b; color: #cbd5e1; cursor: pointer; font-size: 13px; font-weight: 500; }}
    .filter-btn.active {{ background: #38bdf8; color: #0f172a; font-weight: 600; border-color: #38bdf8; }}
    .grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 16px; }}
    .card {{ background: #1e293b; border-radius: 8px; border: 1px solid #334155; padding: 14px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.2); }}
    .card-header {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }}
    .fname {{ font-size: 11px; color: #94a3b8; word-break: break-all; }}
    .badge {{ font-size: 10px; font-weight: 700; padding: 2px 6px; border-radius: 4px; }}
    .badge.good {{ background: #064e3b; color: #34d399; }}
    .badge.stain {{ background: #7c2d12; color: #fb923c; }}
    .badge.recovered {{ background: #701a75; color: #f472b6; }}
    .badge.crease {{ background: #164e63; color: #38bdf8; }}
    .card-meta {{ font-size: 12px; color: #cbd5e1; }}
  </style>
</head>
<body>
  <h1>Fabric Stain Inspection V6 Interactive Gallery</h1>
  <div class="filters">
    <button class="filter-btn active" onclick="filterCards('all')">Show All ({len(rows)})</button>
    <button class="filter-btn" onclick="filterCards('good')">Clean Normals ({int(np.sum(image_labels == 0))})</button>
    <button class="filter-btn" onclick="filterCards('stain')">Strict Stains ({int(tp_s)})</button>
    <button class="filter-btn" onclick="filterCards('recovered')">Recovered Diffuse Stains ({int(np.sum((image_labels == 1) & (~strict_predictions) & stain_predictions))})</button>
    <button class="filter-btn" onclick="filterCards('crease')">Creases Isolated ({int(np.sum(crease_predictions))})</button>
  </div>
  <div class="grid">
    {''.join(html_cards)}
  </div>
...
report = {
    'status': 'passed',
    'scope': 'locked_fabric_stain_image_evaluation_v6',
...
(OUTPUT / 'evaluation_report_v6.json').write_text(json.dumps(report, indent=2))
np.savez_compressed(OUTPUT / 'test_maps_v6_float16.npz',
                    v6_maps=test_v6_maps.astype(np.float16),
...
archive = shutil.make_archive('/kaggle/working/fabric_stain_evaluation_v6_results', 'zip', OUTPUT)
print(json.dumps(report, indent=2))
print('\\nLOCKED FABRIC STAIN EVALUATION V6 PASSED')
